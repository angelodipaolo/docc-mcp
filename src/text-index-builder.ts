...
/**
 * CLI tool to build text search index from DocC archives
 */
async function buildTextIndex() {
  ...
  if (args.length === 0 || args.includes('--help') || args.includes('-h')) {
    console.log(`
DocC Text Search Index Builder

Usage: node dist/text-index-builder.js --archive-path <path> [options]
...
Examples:
  node dist/text-index-builder.js --archive-path ./docs --archive-path ~/my-docs
  node dist/text-index-builder.js --archive-path ~/Library/Developer/Xcode/DerivedData --rebuild
`);
    process.exit(0);
  }
  ...
  console.log('🚀 Starting text search index building...');
  ...
  try {
    ...
    if (rebuild) {
      ...
      console.log(`📖 Loaded existing index: ${stats.totalDocuments} documents across ${stats.archives.length} archives`);
    }
...
// Run the text index builder
buildTextIndex().catch(console.error);